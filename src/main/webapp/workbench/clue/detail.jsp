...
<script type="text/javascript">
	...
	$(function(){
		...
		//为关联的市场活动模态窗口中的搜索框绑定事件,通过回车键触发
		$("#aname").on("keydown",function (event) {
			if(event.keyCode == 13){
				$.ajax({
	...
	function showActivityList() {
		$.ajax({
			url:"workbench/clue/ActivityByClueId.do",
			data:{
				"clueId":"${clue.id}"
			},
			type:"get",
			...
			success:function (data) {
				...
				$.each(data,function (i,n) {
					...
					html += '<td>'+n.endDate+'</td>';
					html += '<td>'+n.owner+'</td>';
					html += '<td><a href="javascript:void(0);" onclick="nubund(\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
				});
